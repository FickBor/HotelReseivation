...
import com.example.hotelreservation.adapter.LocationListview;
import com.example.hotelreservation.bean.CityData;
import com.example.hotelreservation.tools.CityAnalysis;
import com.example.hotelreservation.adapter.LocationListview.OnTouchingLetterChangedListener;
...
public class SelectCityActivity extends Activity {
    ...
    private LocationListview letterListView; // A-Z listview
    private HashMap<String, Integer> alphaIndexer;// 存放存在的汉语拼音首字母和与之对应的列表位置
    private String[] sections;// 存放存在的汉语拼音首字母
    private Handler handler;
    private OverlayThread overlayThread; // 显示首字母对话框
    private ArrayList<CityData> allCity_lists; // 所有城市列表
    private ArrayList<CityData> ShowCity_lists; // 需要显示的城市列表-随搜索而改变
    private ArrayList<CityData> city_lists;// 城市列表
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        allCity_lists = new ArrayList<CityData>();
        letterListView = (LocationListview) findViewById(R.id.MyLetterListView01);
    ...
    public void hotCityInit() {
        CityData city;
        city = new CityData("上海", "");
        allCity_lists.add(city);
        city = new CityData("北京", "");
        allCity_lists.add(city);
        city = new CityData("广州", "");
        allCity_lists.add(city);
        city = new CityData("深圳", "");
        allCity_lists.add(city);
        city = new CityData("武汉", "");
        allCity_lists.add(city);
        city = new CityData("天津", "");
        allCity_lists.add(city);
        city = new CityData("西安", "");
        allCity_lists.add(city);
        city = new CityData("南京", "");
        allCity_lists.add(city);
        city = new CityData("杭州", "");
        allCity_lists.add(city);
        city = new CityData("成都", "");
        allCity_lists.add(city);
        city = new CityData("重庆", "");
        allCity_lists.add(city);
    ...
    private ArrayList<CityData> getCityList() {
        ...
        try {
            ...
            for(int i=0;i<chineseCities.length();i++){
                ...
                CityData city = new CityData(jsonObject.getString("name"), jsonObject.getString("pinyin"));
                list.add(city);
            }
     ...
     * a-z排序
     */
    Comparator comparator = new Comparator<CityData>() {
        @Override
...
//				sb.append(location.getAddrStr());
    ...
    Handler handler2 = new Handler(){
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case SHOWDIALOG:
                    progress = CityAnalysis.showProgress(SelectCityActivity.this, "正在加载数据，请稍候...");
                    break;
                case DISMISSDIALOG:
                    if (progress != null)
                    {
...
//				personList.setAdapter(adapter);
                    ...
                    sh.addTextChangedListener(new TextWatcher() {
                        ...
                        public void afterTextChanged(Editable s) {
                            //搜索符合用户输入的城市名
                            if(s.length()>0){
                                ArrayList<CityData> changecity = new ArrayList<CityData>();
